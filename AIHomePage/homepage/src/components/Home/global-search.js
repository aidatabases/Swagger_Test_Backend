    ...
    console.log("props",props)
      ...
      // const goLogin = () => history.push('login');
        ...
        function handleChange(VariableName){
          var url = 'http://159.65.150.184:9200/ai_economics_v1_1,hello_trade_analytica/_search'
          var query = {
            "query": {
              "bool": {
                "must": [
              ...
              .then( d => {
                if(d.status === 200 ){
                  ...
                  let list = total_data.map(item => {
                    ...
                    if(index === "ai_economics_v1_1"){

                      path = item._source.path
                      if(!path){
                        path = " "
                      }
                      // title = `Path : ${path}       VARIABLE Name -${item._source.VariableName}        PRODUCT : AI-Tr adeAnalytics`
                      title = `Path : ${path}${item._source.VariableName}        PRODUCT : AI-TradeAnalytics`;
                      path = `AI-TradeAnalytics/${path}${item._source.VariableName}`
                    }
                    else if(index == "hello_trade_analytica"){
                      title = `HSCodeName - ${item._source.HSCodeName}      HSCode - ${item._source.HSCode}       PRODUCT : AI-Economics`;
                      path = `AI-Economics/${item._source.HSCode}`
                    }
                    return {
                      'title':title,
                      'label':title,
